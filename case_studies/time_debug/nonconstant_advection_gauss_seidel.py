...
def velocity(t):
    ...
    if args.modes > 0:
        c += fd.sin(pi2*fd.Constant(omegas[0])*t-fd.Constant(0.0))*fd.as_vector([+0.25*fd.sin(1*x*pi2+0.3), +0.20*fd.cos(1*y*pi2-0.9)])
    if args.modes > 1:
        c -= fd.cos(pi2*fd.Constant(omegas[1])*t-fd.Constant(0.7))*fd.as_vector([-0.05*fd.cos(2*x*pi2-0.8), +0.10*fd.sin(2*x*pi2+0.1)])
    if args.modes > 2:
        c += fd.sin(pi2*fd.Constant(omegas[2])*t+fd.Constant(0.6))*fd.as_vector([+0.15*fd.cos(4*x*pi2+0.0), -0.05*fd.sin(4*x*pi2-0.4)])
    if args.modes > 3:
        c -= fd.cos(pi2*fd.Constant(omegas[3])*t-fd.Constant(0.3))*fd.as_vector([-0.03*fd.cos(6*x*pi2-0.9), +0.08*fd.sin(6*x*pi2+0.2)])
    return c
...
def form_function(q, phi, t):
    ...
    un = fd.conditional(fd.dot(u, n) > 0, fd.dot(u, n), 0)
...
parameters.update(circulant_parameters)
...
chunk_aaoform = asQ.AllAtOnceForm(chunk_aaofunc, dt, args.theta,
                                  form_mass, form_function)
# PETSc.Sys.Print('### === --- AAOSolver --- === ###')
# PETSc.Sys.Print('')


def update_time(aaosolver, t=None):
    aaosolver.aaoform.time_update(t=t)
    aaosolver.jacobian_form.time_update(t=t)
...
# global ensemble_rank of rank chunk c (assumes all chunks are equal size)
chunk_root = lambda c: c*args.nslices
...
for j in range(args.nsweeps):
    ...
    if earliest():
        ...
        with chunk_aaoform.F.global_vec_ro() as rvec:
            convergence_residual[0] = rvec.norm()

    # tell everyone else on the global ensemble_comm if they've converge3d
    global_ensemble.ensemble_comm.Bcast(convergence_residual,
    ...
    PETSc.Sys.Print(f">>> Converged time: {converged_time}")
    PETSc.Sys.Print('')
