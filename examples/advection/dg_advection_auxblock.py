...
class AuxBlockPC(fd.AuxiliaryOperatorPC):
    def form(self, pc, v, u):
        ...
        u0 = appctx['u0']
        t0 = appctx['t0']
        d1 = appctx.get(f'aux_{blockid}_d1', appctx['d1'])
        d2 = appctx.get(f'aux_{blockid}_d2', appctx['d2'])

        pc_form_mass = appctx['aux_form_mass']
        pc_form_function = appctx['aux_form_function']

        Vc = v.function_space()
        M = cpx.BilinearForm(Vc, d1, pc_form_mass)
        K = cpx.derivative(d2, partial(pc_form_function, t=t0), u0)
...
appctx = {
    'block_appctx': {
        'aux_form_mass': form_mass,
        'aux_form_function': form_function,
    }
}
...
aux_params = {
    ...
    'ksp_rtol': 1e-4,
    'pc_type': 'python',
    'pc_python_type': f'{__name__}.AuxBlockPC',
    'aux': ilu_params
}

block_parameters = aux_params
# block_parameters = lu_params
