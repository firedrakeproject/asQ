...
w0.interpolate(fd.sin(2*fd.pi*x)*fd.cos(3*fd.pi*y + fd.pi/2))


def coef(t):
    return 2 + fd.sin(t)

#u_exact = exp(-t)*sin(2*pi*x)*cos(3*pi*y)
def Rhs(t):
     u_exact = fd.exp(-t)*fd.sin(2*fd.pi*x)*fd.cos(3*fd.pi*y + fd.pi/2)
     return -u_exact - coef(t)*fd.div(fd.grad(u_exact))
...
def form_function(q, phi, t):
    return coef(t)*fd.inner(fd.grad(q), fd.grad(phi))*fd.dx - Rhs(t)*phi*fd.dx
...
pdg = asQ.paradiag(ensemble=ensemble,
                   ...
                   form_mass=form_mass,
                   w0=w0, dt=dt, theta=args.theta, bcs=bcs,
...
def window_postproc(pdg, wndw):
    ...
    for step in range(aaos.ntimesteps):
        if aaos.layout.is_local(step):
            ...
            q_exact.interpolate(fd.exp(-t)*fd.sin(2*fd.pi*x)*fd.cos(3*fd.pi*y + fd.pi/2))
