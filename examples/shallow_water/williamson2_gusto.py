...
def form_function_asq(u, h, v, q, t):
...
sparameters_diag['diagfft_block_'] = lu_params
...
ucheck, hcheck = wcheck.subfunctions[:]
...
def window_postproc(pdg, wndw):
    ...
    for i in range(pdg.nlocal_timesteps):
        uerrors.dlocal[i], herrors.dlocal[i] = steady_state_test(pdg.aaos.get_field(i))
    ...
    for window_index in range(pdg.ntimesteps):
        ...
        uerr = uerrors.dglobal[window_index]
        herr = herrors.dglobal[window_index]
