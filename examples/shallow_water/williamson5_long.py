...
# the first time-window struggles with the mountain appearing in williamson5
# so we can use the solution after a bit of serial integration as the intial
skip_start = False

if skip_start:
    PETSc.Sys.Print('### === --- Calculating serial start --- === ###')
    PETSc.Sys.Print('')
    ...
    if ensemble.ensemble_comm.rank == 0:
        wguess.split()[0].assign(wserial[-2].split()[0])
        wguess.split()[1].assign(wserial[-2].split()[1])
    if ensemble.ensemble_comm.rank == 1:
        wguess.split()[0].assign(wserial[-1].split()[0])
        wguess.split()[1].assign(wserial[-1].split()[1])

    w0.split()[0].assign(wserial[-3].split()[0])
    w0.split()[1].assign(wserial[-3].split()[1])
...
if skip_start:
    PD.w_alls[0].assign(wguess.split()[0])
    PD.w_alls[1].assign(wguess.split()[1])
...
for w in range(args.nwindows):
    ...
    if w == 0:
        ...
        # PD.snes.setLineSearchDamping(1)
